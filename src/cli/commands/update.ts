/**
 * update command - Update local moon package index from a registry
 */
...
import { CONFIG_FILE_NAME, UPSTREAM } from "../../config/defaults.ts";
...
export function registerUpdateCommand(program: Command): void {
  program
    .command("update")
    .description("Update local moon package index from a registry")
    .option("-r, --registry <url>", "Registry URL (default: uses server.base_url from config)")
    .option("-m, --mooncakes", "Update from official mooncakes.io registry")
    ...
    .action(async (options: UpdateCommandOptions) => {
      try {
        ...
        // --mooncakes flag takes precedence
        if (options.mooncakes) {
          registryUrl = UPSTREAM.URL;
          logger.debug("Using official mooncakes.io registry");
        }
        ...
        if (!registryUrl) {
          ...
          console.error("  3. Use --mooncakes to update from official registry");
          console.error("");
          console.error("Examples:");
          console.error("  moonbit-registry update");
          console.error("  moonbit-registry update --registry http://localhost:8080");
          console.error("  moonbit-registry update --mooncakes");
          process.exit(1);
        }
